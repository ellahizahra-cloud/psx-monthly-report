...
Two independent automations over the same 46-ticker / 9-sector PSX universe
(`tickers.py`). See `.github/workflows/` for their schedules — GitHub
Actions is the durable cron: it runs even when nobody has a session open,
and commits state back to the repo so history survives between runs.
